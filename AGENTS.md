...
- 実装を依頼された場合は、必要な変更、テスト、差分確認まで行う。
- `git commit`、`git push`、PR作成は、明示的に依頼された場合だけ行う。
...
- 実験プロトコルや研究判断を変更した場合は、必要に応じて `notes/decisions/` を更新する。
- 正式実験を実行して結果を確定した場合は、必要に応じて `notes/experiments/` に条件、コマンド、ケース数、結果、出力先を記録する。
